package live.cnaidai.com.ccyb.net;
...
public class OkHttpClientManager {
    ...
    private OkHttpClientManager() {
        ...
        //cookie enabled
        mOkHttpClient.setCookieHandler(new CookieManager(App.getInstance().getMyCookieStore(), CookiePolicy.ACCEPT_ORIGINAL_SERVER));
    ...
    private void deliveryResult(final ResultCallback callback, Request request) {
        //当第一次超时,还没有任何返回数据时,将去检测接口是否存在
        mOkHttpClient.newCall(request).enqueue(new Callback() {
            @Override
            public void onFailure(final Request request, final IOException e) {
                //当网络异常时,直接提示用户检查网络
                if(!NetUtils.isNetworkConnected()){
                    sendNetErrorCallback(request, e, callback);
                    return;
                }
                //当http请求发生异常时,将进行3次重连,超出3此将发出通知
                try {
                    if (requestCount < 3) {
                        if (requestCount == 0){
                            checkInterfaceExist(request, e, callback);
                        }
                        deliveryResult(callback, request);
                        requestCount++;
                    }else{
    ...
    private void checkInterfaceExist(final Request request, final Exception e, final ResultCallback callback) {
        mDelivery.post(new Runnable() {
            @Override
            public void run() {
                if (callback != null)
                    callback.checkInterfaceExist(request, e);
            }
        });
    }


    private void sendNetErrorCallback(final Request request, final Exception e, final ResultCallback callback) {
        mDelivery.post(new Runnable() {
            @Override
            public void run() {
                if (callback != null)
